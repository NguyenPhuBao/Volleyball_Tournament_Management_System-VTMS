...
final class AdminSystemLogRepository
{
    public function list(array $filters, int $limit, int $offset): array
    {
        ...
        $sql .= ' ORDER BY nk.thoigian DESC, nk.idnhatky DESC LIMIT '.(int) $limit.' OFFSET '.(int) $offset;
    ...
    {
        return array_map(
            static fn (object $row): string => (string) $row->bangtacdong,
            DB::select(
    ...
    {
        return array_map(
            static fn (object $row): string => (string) $row->hanhdong,
            DB::select(
