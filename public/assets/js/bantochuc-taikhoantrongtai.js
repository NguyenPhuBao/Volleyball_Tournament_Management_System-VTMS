(() => {
    const root = document.querySelector(".organizer-referee-accounts");

    if (!root) {
        return;
    }

    const accountsApi = root.dataset.accountsApi || "/api/organizer/referee-accounts";
    ...
    const fields = {
        ...
        refStatus: document.getElementById("m_refStatus"),
    ...
    function badgeClass(status) {
        ...
        if (status === "DA_HUY" || status === "TAM_KHOA" || status === "NGUNG_HOAT_DONG") {
            return "bad";
        }
    ...
    function renderRows() {
        ...
        if (accounts.length === 0) {
            tbody.innerHTML = '<tr><td colspan="8" class="empty">Không có tài khoản trọng tài phù hợp.</td></tr>';
            return;
        }
    ...
    async function loadAccounts(showLoading = true) {
        ...
        try {
            ...
            accounts = [];
            renderRows();
            showMessage(error.message || "Không thể tải danh sách tài khoản trọng tài.");
        }
    }
    ...
    function fillDetail(account) {
        ...
        fields.level.value = referee.capbac || "";
        fields.refStatus.value = statusLabel(referee.trangthai);
    ...
    async function openDetail(accountId) {
        ...
        try {
            ...
            showMessage(error.message || "Không thể tải chi tiết tài khoản trọng tài.");
        }
    }
    ...
    async function processCurrent(action) {
        ...
        if (!window.confirm(`Xác nhận ${verb} tài khoản trọng tài này?`)) {
            return;
        }
        ...
        try {
            ...
            closeModal();
            showMessage(payload.message || "Cập nhật tài khoản trọng tài thành công.", true);
            await loadAccounts(false);
        } catch (error) {
            showModalAlert(error.message || "Không thể cập nhật tài khoản trọng tài.");
            setBusy(false);
        }
    }
